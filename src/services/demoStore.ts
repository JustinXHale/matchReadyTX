  ...
  if (alexCmo && alexCmoUid) {
    existing.push({
      ...
      cmoPayload: {
        ...
        playedLike: 'Women’s D2 as listed',
        matchKind: 'League Match',
        gameTemperature: 4,
        contestBalance: 4,
        complexityFactors: ['NO MAJOR FAVORS. GREAT DAY'],
        attendedInPerson: 'yes',
        keep: 'Calm captain management when the game heated up.',
        start: 'Scan earlier at the breakdown before committing.',
        stop: 'Lingering in the passing lane after the tackle.',
        overallComment:
          'Strong control game. Continue scanning early at the breakdown.',
        assessedRating: 8,
        gradingConfidence: 4,
        gradingRationale: 'Consistent patterns across set piece and open play.',
      },
    });
  }
  ...
  if (aboutAlex && aboutAlexCmoUid) {
    existing.push({
      ...
      cmoPayload: {
        ...
        playedLike: 'Club D1 with D2 skill mix after injuries',
        matchKind: 'League Match',
        gameTemperature: 3,
        contestBalance: 3,
        complexityFactors: ['short benches/injuries'],
        attendedInPerson: 'yes',
        keep: 'Material advantage that produced a try.',
        start: 'Hold midfield depth in the second half.',
        stop: 'Getting stuck at the breakdown.',
        overallComment:
          'Solid outing as MO. Keep scanning early at the breakdown.',
        assessedRating: 7,
        gradingConfidence: 5,
        gradingRationale: 'Clear patterns; advantage and big decisions stood out.',
      },
    });
  }
  ...
  for (let i = 0; i < cmoTargets.length; i += 1) {
    ...
    existing.push({
      ...
      cmoPayload: {
        ...
        playedLike: match.level,
        matchKind: 'League Match',
        gameTemperature: 3,
        contestBalance: 4,
        complexityFactors: ['NO MAJOR FAVORS. GREAT DAY'],
        attendedInPerson: 'yes',
        keep: 'Clear communication with captains.',
        start: 'Scan earlier at the breakdown.',
        stop: 'Over-explaining secondary signals.',
        overallComment: 'Solid outing — continue scanning early at the breakdown.',
        assessedRating: 6 + (i % 4),
        gradingConfidence: 4,
        gradingRationale: 'In line with the appointed level.',
      },
    });
  }
