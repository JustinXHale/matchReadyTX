...
export function SchedulerQueuesWorkPage() {
  ...
  }, [state.fixtureRequests, divisionActive, genderFilter, levelFilter]);
  ...
  const raiseHand = useMemo(() => {
    ...
    if (!divisionActive) return pending;
    return pending.filter((r) => {
      const m = state.matches.find((x) => x.id === r.matchId);
      return (
        m != null &&
        matchMatchesDivisionFilters(
          m,
  ...
  const needsOfficials = useMemo(() => {
    const list = matchesNeedingOfficials(state.matches);
  ...
  const needsReassignment = useMemo(() => {
    const list = matchesNeedingReassignment(state.matches);
    if (!divisionActive) return list;
    return list.filter((m) =>
      matchMatchesDivisionFilters(
        m,
  ...
  const t72 = useMemo(() => {
    const list = matchesT72Due(state.matches);
    if (!divisionActive) return list;
    return list.filter((m) =>
      matchMatchesDivisionFilters(
        m,
  ...
  return (
    <>
      ...
      <GlobalDivisionFilters
        ...
        ariaLabel="Filter queues by division"
      />
      {divisionActive && filteredActionable === 0 && (
        <p className="rs-match-card__meta">
          No queue items for this division. Clear Men/Women or level chips to see
          everything.
        </p>
      )}
